...
- точку минимума $x^{\ast}$, для которой длина финального интервала неопределённости не превышает $\varepsilon$ (найденная точка отличается от истинного минимума не более чем на $\varepsilon$);
- значение функции в точке минимума $f(x^{\ast})$;
...
Условие минимума: $f'(x^{\ast}) = 0$, то есть $x^{\ast}\cos x^{\ast} = \sin x^{\ast}$.
...
На графике (файл `images/unimodality.png`, построен скриптом `plot.py`) видно, что на отрезке $[-5, -4]$ функция имеет **ровно один минимум** — при $x^{\ast} \approx -4{,}4933$. Функция убывает левее этой точки и возрастает правее, что наглядно подтверждает унимодальность и применимость обоих методов.
...
5. Найти $x^{\ast} = x_k$, где $k = \arg\min_i f(x_i)$.
...
**Гарантия точности:** так как шаг сетки $h \leq \varepsilon$, реальный минимум находится не дальше $\varepsilon$ от найденного: $|x^{\ast} - x_{\text{найд}}| \leq \varepsilon$.
...
3. Вернуть $x^{\ast} = \dfrac{a + b}{2}$, вычислить $f^{\ast} = f(x^{\ast})$ — **1 финальное обращение**.
...
> **Примечание.** Стандартная формула из учебников даёт $n + 2$ (без финального вызова), когда $x^{\ast}$ определяется как середина конечного интервала без вычисления $f$. В данной реализации после завершения цикла дополнительно вычисляется $f\!\left(\tfrac{a+b}{2}\right)$ для возврата значения функции в точке минимума — отсюда $+1$ и итоговое $n + 3$.
...
| ε | x* | f(x*) | Вызовов (факт) | Вызовов (теория) |
|---|---|---|---|---|
...
| ε | Равном. поиск (вызовов) | Золотое сечение (вызовов) | Разница |
|---|---|---|---|
| 0,1 | 11 | 8 | в 1,4 раза |
...
- **Равномерный поиск:** 11 → 101 → 1001 — при каждом десятикратном уменьшении ε число вызовов возрастает **в 10 раз** (линейный рост, O(1/ε)).
- **Золотое сечение:** 8 → 13 → 18 — при каждом десятикратном уменьшении ε число вызовов увеличивается лишь **на 5** (логарифмический рост, O(log(1/ε))).
...
Для дифференцируемой функции в точке минимума $x^{\ast}$ необходимо:
$$f'(x^{\ast}) = 0$$

Производная функции:
$$f'(x) = x\cos x - \sin x$$

Используем наиболее точный результат, полученный методом золотого сечения при $\varepsilon = 0{,}001$: $x^{\ast} = -4{,}493336$.
...
Значение $f'(x^{\ast}) \approx 0{,}001$ близко к нулю. Отличие от нуля объясняется тем, что $x^{\ast} = -4{,}493336$ — **приближённое** значение с точностью $\varepsilon = 0{,}001$. Необходимое условие **выполнено** с точностью, соответствующей требуемой погрешности.

### 5.2 Достаточное условие (знак второй производной)

Для того чтобы точка $x^{\ast}$ была минимумом, необходимо $f''(x^{\ast}) > 0$.

Вычислим вторую производную:
$$f''(x) = \frac{d}{dx}(x\cos x - \sin x) = \cos x - x\sin x - \cos x = -x\sin x$$

Подставляем $x^{\ast} = -4{,}493336$:
$$f''(-4{,}493336) = -(-4{,}493336) \cdot 0{,}9761 = 4{,}493336 \cdot 0{,}9761 \approx +4{,}386$$

Так как $f''(x^{\ast}) \approx 4{,}386 > 0$, точка $x^{\ast}$ является точкой **локального минимума**. Условие выполнено.
...
| Разность $\lvert x_1^{\ast} - x_2^{\ast} \rvert$ | 0,000336 < ε | — |

Оба метода сошлись к одному и тому же значению функции $f^{\ast} = -4{,}820572$, а расхождение в найденной точке не превышает требуемой точности $\varepsilon = 0{,}001$. Это **взаимная верификация** результата двумя независимыми алгоритмами.

### 5.4 Итоговая проверка

Убедимся, что $f(x^{\ast})$ действительно является минимальным значением, а не максимумом, сравнив со значениями на концах отрезка:

$$f(-5) = (-5)\sin(-5) + 2\cos(-5) = (-5)(0{,}9589) + 2(0{,}2837) = -4{,}794 + 0{,}567 = -4{,}227$$
$$f(-4) = (-4)\sin(-4) + 2\cos(-4) = (-4)(0{,}7568) + 2(-0{,}6536) = -3{,}027 - 1{,}307 = -4{,}334$$
$$f(x^{\ast}) \approx -4{,}821$$

Значение $f(x^{\ast}) = -4{,}821$ **меньше** значений на обоих концах отрезка. Это подтверждает, что найденная точка является глобальным минимумом на $[-5, -4]$.
...
**Найденный минимум:** $x^{\ast} \approx -4{,}4933$, $f^{\ast} \approx -4{,}8206$.

**Достоверность подтверждена:**
- необходимое условие: $f'(x^{\ast}) \approx 0{,}001 \approx 0$;
- достаточное условие: $f''(x^{\ast}) \approx 4{,}386 > 0$;
- совпадение результатов обоих методов при $\varepsilon = 0{,}001$.

**Сравнение эффективности методов:**

| ε | Равном. поиск | Золотое сечение | Выигрыш |
|---|---|---|---|
| 0,1 | 11 | 8 | ×1,4 |
| 0,01 | 101 | 13 | ×7,8 |
| 0,001 | 1001 | 18 | **×55,6** |
...
- **Равномерный поиск** (O(1/ε)) оправдан при очень грубой точности (ε ~ 0,1) или когда вычисление f практически бесплатно. Его главное достоинство — простота реализации и отсутствие требования унимодальности (находит глобальный минимум на сетке). Недостаток: число вычислений растёт линейно, что при ε = 0,001 уже требует 1001 вызова.

- **Метод золотого сечения** (O(log(1/ε))) предпочтителен во всех случаях, когда функция унимодальна и требуется высокая точность. Каждое десятикратное уменьшение ε добавляет лишь ≈5 новых вызовов. При ε = 0,001 — всего 18 вызовов против 1001 у равномерного поиска (выигрыш в **55 раз**). Ограничение: требует унимодальности на отрезке поиска.